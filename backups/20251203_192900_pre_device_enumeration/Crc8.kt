...
/**
 * CRC8 implementation matching decompiled C# code
 * Uses lookup table with initial value 85
 */
object Crc8 {
    private const val RESET_VALUE: Byte = 85
    
    // CRC8 lookup table from decompiled C# code
    // Using explicit byte array initialization to avoid integer literal issues
    ...
    fun calculate(data: ByteArray, offset: Int, count: Int): Byte {
        var crc = RESET_VALUE.toInt() and 0xFF
        
        var idx = offset
        var remaining = count
        
        // Process odd bytes first
        if ((remaining and 1) != 0) {
            crc = TABLE[(crc xor (data[idx].toInt() and 0xFF)) and 0xFF].toInt() and 0xFF
            idx++
            remaining--
        }
    ...
    /**
     * Update CRC8 with a single byte
     */
