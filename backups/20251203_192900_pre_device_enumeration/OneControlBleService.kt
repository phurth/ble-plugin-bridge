...
class OneControlBleService : Service() {
    ...
    // Active stream reading (like official app's ReadAsync loop)
    ...
    private fun updateNotification(text: String) {
        ...
        // Also broadcast log message to MainActivity
        broadcastLog(text)
    }
    ...
    private fun sendInitialCanCommand() {
        ...
        try {
            ...
            val command = encodeGetDevicesCommand(commandId, deviceTableId)
            val commandHex = command.joinToString(" ") { "%02X".format(it) }
            if (deviceTableId == 0x00.toByte()) {
                Log.w(TAG, "📤 Initial GetDevices: Using DeviceTableId=0x00 (will update when GatewayInformation received)")
            }
            Log.d(TAG, "📤 Initial GetDevices: CommandId=0x${commandId.toString(16)}, DeviceTableId=0x${deviceTableId.toString(16)}, raw=$commandHex")
                ...
                try {
                    ...
                    while (notificationQueue.isNotEmpty() && !shouldStopStreamReading) {
                        ...
                        try {
                            ...
                            val preview = notificationData.take(20).joinToString(" ") { "%02X".format(it) }
                            Log.i(TAG, "📥 First 20 bytes: $preview")
                            val hasFrameDelimiter = notificationData.contains(0x00.toByte())
                        ...
                        if (bytesProcessed > 0 && notificationData.isNotEmpty()) {
    ...
    private fun handleCharacteristicNotification(characteristic: BluetoothGattCharacteristic) {
        ...
        if (uuid == Constants.CAN_READ_CHAR_UUID.lowercase() || uuid == Constants.DATA_READ_CHAR_UUID.lowercase()) {
            ...
            // For other characteristics, just log
            Log.d(TAG, "📨 ${uuid}: ${data.joinToString(" ") { "%02X".format(it) }}")
        }
        
        // Publish to MQTT with appropriate topic
        val topic = when {
    ...
    private fun processMyRvLinkEvent(data: ByteArray) {
        ...
        // Check if it's a command response first
    ...
    private fun handleMyRvLinkEvent(event: MyRvLinkEvent) {
        when (event.eventType) {
            ...
            MyRvLinkEventType.RelayBasicLatchingStatusType2 -> {
                ...
                // Similar structure to Type1, handle if needed
            }
        ...
        try {
            ...
            mqttClient?.setCallback(object : MqttCallback {
                ...
                override fun messageArrived(topic: String?, message: MqttMessage?) {
                    Log.d(TAG, "MQTT message: $topic = ${message?.toString()}")
    ...
    private fun publishMqtt(topic: String, payload: String) {
        if (!mqttConnected || mqttClient == null) {
            Log.d(TAG, "MQTT not connected, skipping publish: $topic")
            return
        }
        ...
        try {
            ...
            Thread {
                try {
                    ...
                    Log.d(TAG, "MQTT published: $fullTopic = $payload")
                } catch (e: Exception) {
    ...
    private fun handleRelayBasicLatchingStatusType1(data: ByteArray) {
        ...
        for (status in statuses) {
            ...
            deviceStatuses[key] = DeviceStatus.Relay(status.deviceTableId, status.deviceId, status.isOn)
            
            Log.i(TAG, "🔌 Relay ${status.deviceId} (TableId=${status.deviceTableId}): ${if (status.isOn) "ON" else "OFF"}")
            broadcastLog("🔌 Relay ${status.deviceId}: ${if (status.isOn) "ON" else "OFF"}")
            
            // Publish to MQTT
            publishMqtt("device/${status.deviceTableId}/${status.deviceId}/state", if (status.isOn) "ON" else "OFF")
    ...
    private fun handleDimmableLightStatus(data: ByteArray) {
        ...
        for (status in statuses) {
            ...
            // Publish to MQTT
            publishMqtt("device/${status.deviceTableId}/${status.deviceId}/state", if (isOn) "ON" else "OFF")
            publishMqtt("device/${status.deviceTableId}/${status.deviceId}/brightness", brightness.toString())
    ...
    private fun handleRgbLightStatus(data: ByteArray) {
        ...
        for (status in statuses) {
            ...
            // RGB status parsing would go here if needed
        }
    }
