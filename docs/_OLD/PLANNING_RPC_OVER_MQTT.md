...
## Already Implemented (No Longer In Scope)

The following features from the original plan are already implemented inline in the plugins and do **not** need to be part of the RPC effort:

### Optimistic State & Suppression

Plugins already handle optimistic state and telemetry suppression inline:

- **OneControl HVAC** (`OneControlDevicePlugin.kt`): `PendingHvacCommand` tracks commanded values with a 60s suppression window for setpoints, 8s for mode/fan. `sendHvacCommand()` does an optimistic MQTT publish immediately after BLE write. `handleHvacStatus()` checks pending commands and suppresses mismatching telemetry during the window.
- **OneControl Lights**: Debounced command sending with state tracking.
- **EasyTouch**: Similar pattern for setpoint commands.

Centralizing this into an `OptimisticStateManager` was considered but is unnecessary — the per-plugin inline approach works, is already tested, and avoids the risk of a cross-cutting refactor.

### Metadata Refresh

Implemented in v2.6.2 (commit `49e4bdd`):
- `refreshMetadata()` method in `OneControlDevicePlugin` resets metadata guards and re-sends `GetDevicesMetadata` over BLE
- Exposed as an HA button entity (`entity_category: diagnostic`) via MQTT discovery
- Subscribes to `{baseTopic}/command/refresh_metadata`
- No RPC framework needed — works via existing command topic infrastructure

When RPC is implemented, `OneControl.RefreshMetadata` can be added as a thin wrapper around the existing method to provide confirmed response.
