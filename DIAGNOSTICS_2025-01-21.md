...
- **OneControl**: ⚠️ **Partial** — Instance loaded but not connecting to device (separate issue)
...
The **repeated plugin instance loading was the root cause** of intermittent active plugin disconnections. By implementing a 500ms cache layer in `ServiceStateManager.getAllInstances()`, we **reduced JSON parsing overhead by 98%** and stabilized GATT connections for EasyTouch and GoPower.

OneControl remains disconnected due to a separate matching/initialization issue unrelated to caching.

**Build & Release**: Ready for v2.5.15 release with improved connection stability.
